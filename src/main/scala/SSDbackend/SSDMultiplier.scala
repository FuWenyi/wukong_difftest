...
class XSFuOutput(val len: Int) extends Bundle {
  val data = UInt(len.W)
}

class XSFunctionUnitInput(val len: Int) extends Bundle {
...
abstract class XSFunctionUnit(len: Int = 64) extends Module {

  val io = IO(new XSFunctionUnitIO(len))
...
//  val rdyVec = (Array.fill(latency - 1)(Wire(Bool())) :+ io.out.ready) :+ WireInit(true.B)
  val rdyVec = Array.fill(latency+1)(io.out.ready)

  // if flush(0), valid 0 will not given, so set flushVec(0) to false.B
  val flushVec = Array.fill(latency+1)(false.B)
...
class ArrayMultiplier(len: Int) extends AbstractMultiplier(len) with HasPipelineReg {
