...
abstract class AXI4SlaveModulexs[T <: Data]
(
...
class AXI4SlaveModuleImp[T<:Data](outer: AXI4SlaveModulexs[T])
...
//  val timer = GTimer()
  when(in.ar.fire){
    /*Debug(p"[ar] addr: ${Hexadecimal(in.ar.bits.addr)} " +
      p"arlen:${in.ar.bits.len} arsize:${in.ar.bits.size} " +
      p"id: ${in.ar.bits.id}\n"
    )*/
  }
  when(in.aw.fire){
    /*Debug(p"[aw] addr: ${Hexadecimal(in.aw.bits.addr)} " +
      p"awlen:${in.aw.bits.len} awsize:${in.aw.bits.size} " +
      p"id: ${in.aw.bits.id}\n"
    )*/
  }
  when(in.w.fire){
    //Debug(p"[w] wmask: ${Binary(in.w.bits.strb)} last:${in.w.bits.last} data:${Hexadecimal(in.w.bits.data)}\n")
  }
  when(in.b.fire){
    //Debug(p"[b] id: ${in.b.bits.id}\n")
  }
  when(in.r.fire){
    //Debug(p"[r] id: ${in.r.bits.id} data: ${Hexadecimal(in.r.bits.data)}\n")
  }
